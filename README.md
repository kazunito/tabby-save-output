...
## インストール手順（リリース版・推奨／ビルド不要）

ビルド済みの zip を Tabby のプラグインフォルダに解凍するだけで使えます。Node.js や git は不要です。

**1. zip をダウンロード**

[Releases ページ](https://github.com/kazunito/tabby-save-output/releases) から `tabby-save-output-<version>.zip`（例: `tabby-save-output-4.0.0.zip`）をダウンロードします。

> ⚠️ **`Source code (zip)` ではありません。** ビルド済み本体（`dist/`）が入っているのは Release に添付された `tabby-save-output-<version>.zip` の方です。GitHub が自動生成する `Source code (zip)` には `dist/` が含まれず、解凍しても動作しません。

**2. Tabby のプラグインフォルダに解凍**

アーカイブを開くと `tabby-save-output/` フォルダが出てきます。これを OS ごとの `plugins/node_modules/` 配下に丸ごと配置します。

**macOS:**
```bash
mkdir -p ~/Library/Application\ Support/tabby/plugins/node_modules
unzip -o ~/Downloads/tabby-save-output-4.0.0.zip \
  -d ~/Library/Application\ Support/tabby/plugins/node_modules/
```

**Linux:** `~/.config/tabby/plugins/node_modules/` に同様に解凍
**Windows:** `%APPDATA%\tabby\plugins\node_modules\` に `tabby-save-output` フォルダを配置

**3. 配置を確認**

```bash
ls ~/Library/Application\ Support/tabby/plugins/node_modules/tabby-save-output/
# package.json と dist/ の両方が見えれば OK
```

**4. Tabby を完全終了 → 再起動**

Settings に **Save Output** タブが現れれば成功です。

---

## インストール手順（ソースからビルド・開発者向け）
...
配布用の zip は `npm run pack` で生成します。`release/tabby-save-output-<version>.zip` が作られます。

```bash
npm run pack
# → release/tabby-save-output-4.0.0.zip
```

中身はビルド済み `dist/` と `package.json` / `README.md` だけで、アーカイブのトップは `tabby-save-output/` フォルダです。利用者は `plugins/node_modules/` に解凍するだけで動作します。

この zip を GitHub Release に **アセットとして添付** してください（`Source code (zip)` は `dist/` を含まないため配布には使えません）。
